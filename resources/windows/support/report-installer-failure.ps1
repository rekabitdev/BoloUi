...
$log = $LogPath
if (-not $log) {
  $log = Join-Path $env:TEMP 'boloui-installer-fallback-log.jsonl'
}
$logFileName = Split-Path -Leaf $log
$statusPath = Join-Path $env:TEMP 'boloui-installer-report.json'
...
function Read-BoloUiAnalyticsId {
  $candidates = @()
  if ($env:APPDATA) {
    $candidates += (Join-Path $env:APPDATA 'BoloUi\analytics.json')
  }
  if ($env:LOCALAPPDATA) {
    $candidates += (Join-Path $env:LOCALAPPDATA 'BoloUi\analytics.json')
  }
...
function Get-OptionalHandleDiagnostics {
  ...
  $outPath = Join-Path $env:TEMP ('boloui-handle-' + [guid]::NewGuid().ToString('N') + '.out')
  $errPath = Join-Path $env:TEMP ('boloui-handle-' + [guid]::NewGuid().ToString('N') + '.err')
...
function New-ReportDetailsText(
  ...
  $lines.Add('--------------------------------')
  $lines.Add('BoloUi installer failure ' + $code)
  $lines.Add('--------------------------------')
  $lines.Add('')
  if ($eventId) {
...
try {
  ...
  $userId = Read-BoloUiAnalyticsId
  $eventPayload = @{
    message = ('installer-failure ' + $Code)
    level = 'error'
    platform = 'other'
    release = $Release
    logger = 'boloui.installer'
  ...
  Show-ReportMessage ('BoloUi installer report sent.' + [Environment]::NewLine + [Environment]::NewLine + 'Tip: If you can get in touch with the BoloUi team, press [ Ctrl + C ] in this dialog to copy details, then send them to us via social media, email, or a GitHub issue:' + [Environment]::NewLine + 'https://github.com/iOfficeAI/BoloUi/issues' + [Environment]::NewLine + [Environment]::NewLine + $reportDetails) 'Information'
} catch {
  ...
  Show-ReportMessage ('BoloUi installer report failed.' + [Environment]::NewLine + [Environment]::NewLine + 'Status: ' + $statusPath + [Environment]::NewLine + 'Log: ' + $log) 'Exclamation'
}
