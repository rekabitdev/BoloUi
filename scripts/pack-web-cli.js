...
const tarballName = `boloui-web-${version}-${normalizedPlatform}-${normalizedArch}.tar.gz`;
...
const tarballContentDir = path.join(stagingDir, 'boloui-web');
...
const executableName = platform === 'win32' ? 'boloui-web.exe' : 'boloui-web';
...
// `boloui-web version` match the tarball filename.
...
execSync(`tar -czf ${path.basename(tarballPath)} -C ${stagingDir} boloui-web`, {
