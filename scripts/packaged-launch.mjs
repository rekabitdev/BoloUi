...
async function main() {
  ...
  if (shouldClean) {
    await killProcessByName('BoloUi.exe');
    await killProcessByName('BoloUi');
  ...
  const env = {
    ...process.env,
    BOLOUI_EXTENSIONS_PATH: path.join(projectRoot, 'examples'),
  };

  console.log(`[packaged-launch] executable: ${packaged.executablePath}`);
  console.log(`[packaged-launch] cwd: ${packaged.cwd}`);
  console.log(`[packaged-launch] BOLOUI_EXTENSIONS_PATH: ${env.BOLOUI_EXTENSIONS_PATH}`);
