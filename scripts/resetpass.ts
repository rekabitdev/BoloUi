 ...
 *   BOLOUI_DATA_DIR=/x bun run resetpass
...
import { startBackend, stopBackend } from '@boloui/web-host';
 ...
 * See the comment there for why the default is `~/.boloui-web*` (not `~/.boloui*`).
 */
 ...
 * desktop side). Callers can override with `--port` / `BOLOUI_PORT` to match
 ...
 * pass --port / BOLOUI_PORT if they launched webui on a non-default one.
 */
