param(
  ...
  [string]$WorktreeRoot = (Join-Path $PSScriptRoot '..\..\boloui-build-worktrees'),
...
function New-BuildCommandFile([string]$WorktreePath, [string]$Version, [string]$Dsn, [string]$LocalAioncoreBinary, [string]$LocalAioncoreBundleDir) {
  ...
  $lines = @(
    ...
    '$env:BOLOUI_DEBUG_AUTO_UPDATE_CURRENT_VERSION = ''' + $Version + '''',
    '$env:ELECTRON_BUILDER_COMPRESSION_LEVEL = ''1''',
    '$env:BOLOUI_BACKEND_LOCAL_BINARY = ''' + ($LocalAioncoreBinary -replace "'", "''") + '''',
    '$env:BOLOUI_BACKEND_LOCAL_BUNDLE_DIR = ''' + ($LocalAioncoreBundleDir -replace "'", "''") + '''',
...
function Wait-BuildProcess($Build, [int]$TimeoutSeconds) {
  ...
  $source = Join-Path $Build.worktreePath "out\BoloUi-$($Build.version)-win-x64.exe"
  ...
  $target = Join-Path $script:OutputDirResolved "BoloUi-$($Build.version)-win-x64.exe"
  Copy-Item -LiteralPath $source -Destination $target -Force
...
if (-not (Test-Path -LiteralPath (Join-Path $localAioncoreBundleDir 'managed-resources') -PathType Container)) {
  $localAioncoreBundleDir = Join-Path $env:LOCALAPPDATA 'Programs\BoloUi\resources\bundled-aioncore\win32-x64'
}
...
try {
  foreach ($version in $buildVersions) {
    $worktreePath = Join-Path $runRoot "BoloUi-$version"
