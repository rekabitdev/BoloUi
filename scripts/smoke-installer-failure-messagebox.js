...
const INSTALLER_ERROR_SCENARIOS = [
  {
    ...
    defineName: 'BOLOUI_E_UNINSTALLER_COPY_OR_REBUILD_FAILED',
    code: 'E1001',
    message: 'BoloUi could not repair the installed uninstaller.',
    action: 'Close BoloUi, restart Windows if needed, then run this installer again.',
    diagnostics:
  ...
  },
  {
    ...
    defineName: 'BOLOUI_E_OLD_UNINSTALL_FAILED',
    code: 'E1002',
    message: 'The previous BoloUi uninstaller returned an error.',
    action:
  ...
  },
  {
    ...
    defineName: 'BOLOUI_E_INSTALL_DIR_REMOVE_OR_LOCKED',
    code: 'E1003',
    message: 'BoloUi could not remove or replace the previous installation directory.',
    action: 'Close BoloUi and any program using the install folder, then run this installer again.',
    diagnostics: 'scenario=install-dir-remove-or-locked phase=atomic-failed failedPath=install-dir',
  },
  {
    id: 'extract-failed',
    defineName: 'BOLOUI_E_EXTRACT_FAILED',
    code: 'E1010',
    message: 'BoloUi could not extract the application files correctly.',
    action: 'Download a fresh installer and run it again.',
    diagnostics: 'scenario=extract-failed phase=extract method=zip missing=BoloUi.exe',
  },
  {
    id: 'disk-insufficient',
    defineName: 'BOLOUI_E_DISK_INSUFFICIENT',
    code: 'E1020',
    message: 'BoloUi cannot continue because the target disk does not have enough free space.',
  ...
  },
  {
    ...
    defineName: 'BOLOUI_E_BUNDLED_AIONCORE_INCOMPLETE',
    code: 'E1030',
    message: 'BoloUi installed, but the bundled AionCore resources are incomplete.',
    action: 'Download a fresh installer and run it again.',
  ...
  },
  {
    ...
    defineName: 'BOLOUI_E_CORE_APP_FILES_INCOMPLETE',
    code: 'E1031',
    message: 'BoloUi installation is incomplete because a required application file is missing.',
    action: 'Reinstall BoloUi or download a newer installer.',
    diagnostics: 'scenario=core-app-files-incomplete phase=verify-required-file missing=resources/app.asar',
  },
  {
    id: 'arch-mismatch',
    defineName: 'BOLOUI_E_ARCH_MISMATCH',
    code: 'E1040',
    message: 'Installation package architecture mismatch.',
    action: 'Download the BoloUi installer that matches this Windows architecture, then run it again.',
    diagnostics: 'scenario=arch-mismatch phase=arch-check target=x64 actual=arm64',
  },
  {
    id: 'active-installer-conflict',
    defineName: 'BOLOUI_E_ACTIVE_INSTALLER_CONFLICT',
    code: 'E1050',
    message: 'Another BoloUi installer appears to still be active.',
  ...
  },
  {
    ...
    defineName: 'BOLOUI_E_REGISTRY_STATE_INVALID',
    code: 'E1060',
    message: 'BoloUi found an invalid previous-install registry state.',
    action: 'Uninstall the old BoloUi from Windows Settings, then run this installer again.',
  ...
  },
  {
    ...
    defineName: 'BOLOUI_E_ACTIVE_MARKER_WRITE_FAILED',
    code: 'E1070',
    message: 'BoloUi could not write the active-installer marker.',
  ...
  },
  {
    ...
    defineName: 'BOLOUI_E_INVALID_INSTALL_PATH',
    code: 'E1090',
...
function copyHarnessProject(projectRoot) {
  ...
  writeFileSync(path.join(supportDir, '_sentry-dsn.generated.nsh'), '!define BOLOUI_SENTRY_DSN ""\n', 'utf8');
}
...
function readInstallerErrorDefinitions() {
  ...
  const definitions = Array.from(source.matchAll(/!define\s+(BOLOUI_E_[A-Z0-9_]+)\s+"(E\d{4})"/g), (match) => ({
    defineName: match[1],
    code: match[2],
  }));
  if (definitions.length === 0) {
    throw new Error('No BOLOUI_E_* installer error codes found.');
  }
  return definitions;
}
...
function Find-FailureWindow([string]$Code, [int]$TimeoutSec = 90) {
  ...
  do {
    ...
    foreach ($window in $windows) {
      ...
      if ($text -like "*BoloUi installation failed ($Code)*" -or
...
try {
  ...
  foreach ($required in @(
    "BoloUi installation failed ($Code)",
    "scenario=$ScenarioId",
    'Suggested action:',
    'Diagnostics:',
    'Installer log:',
...
function createHarnessNsi({ exePath, logPath, projectRoot, scenario }) {
  ...
  return `
Unicode true
Name "BoloUi Failure MessageBox Smoke"
...
Section
  StrCpy $INSTDIR "$TEMP\\BoloUi-messagebox-smoke"
  StrCpy $BoloUiSessionId "smokembox-${nsisQuote(scenario.code)}"
...
function runHarness({ autoDecline, compileOnly, makensis, scenario }) {
  const { code } = scenario;
  const root = mkdtempSync(path.join(tmpdir(), `boloui-failure-messagebox-${code}-`));
  const projectRoot = path.join(root, 'project');
  const nsiPath = path.join(root, 'boloui-failure-messagebox-smoke.nsi');
  const exePath = path.join(root, 'boloui-failure-messagebox-smoke.exe');
  const logPath = path.join(
    process.env.TEMP || tmpdir(),
    `boloui-installer-messagebox-smoke-${code}-${new Date().toISOString().replace(/[-:]/g, '').replace(/\..+$/, '').replace('T', '-')}-log.jsonl`
  );
  const automationPath = path.join(root, 'auto-decline.ps1');
  const reportStatusPath = path.join(process.env.TEMP || tmpdir(), 'boloui-installer-report.json');
  ...
  try {
    ...
    if (autoDecline) {
      ...
      if (autoDecline === 'consent') {
        ...
        if (typeof status.copyText !== 'string' || !status.copyText.includes(`BoloUi installer failure ${code}`)) {
