...
export INSTALL_DIR="/tmp/boloui-web-smoke-test"
...
if [[ ! -x "${INSTALL_DIR}/boloui-web" ]]; then
    echo "❌ CLI executable not found or not executable: ${INSTALL_DIR}/boloui-web"
    exit 1
fi
echo "✓ CLI executable exists"

if [[ ! -L "${BIN_DIR}/boloui-web" ]]; then
    echo "❌ Symlink not found: ${BIN_DIR}/boloui-web"
    exit 1
fi
...
VERSION_OUTPUT=$(boloui-web version 2>&1 || echo "")
