 ...
 * parsing the electron-log file for [BoloUi:ready] / [BoloUi:init] /
 * [BoloUi:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  wallTotalMs: number;
  // Parsed from [BoloUi:ready] marks
  ...
  // Parsed from [BoloUi:init] marks
  initTotalMs: number;
  // Parsed from [BoloUi:process] marks
...
function getLogFilePath(): string {
  ...
  if (process.platform === 'darwin') {
    candidates.push(
      path.join(os.homedir(), 'Library', 'Logs', 'BoloUi-Dev', `${today}.log`),
      path.join(os.homedir(), 'Library', 'Logs', 'BoloUi', `${today}.log`)
    );
  } else if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    candidates.push(path.join(appData, 'BoloUi', 'logs', `${today}.log`));
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'BoloUi', 'logs', `${today}.log`));
  }
...
// Matches: [BoloUi:ready] <label> +<ms>ms
// Matches: [BoloUi:init]  <label> +<ms>ms
// Matches: [BoloUi:process] <label> +<ms>ms
const MARK_REGEX = /\[BoloUi:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[BoloUi] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[BoloUi] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[BoloUi] Showing main window')) marks.logs.showingMainWindow = true;
  }
