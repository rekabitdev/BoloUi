 ...
 *
 * Env vars:
 *   BOLOUI_PORT           : static server port (default 33000)
 *   BOLOUI_HOST           : listen host; set to 0.0.0.0 to imply --remote
 *   BOLOUI_ALLOW_REMOTE   : "1"/"true" to expose to LAN
 *   BOLOUI_DATA_DIR       : override userData path (default Electron-compatible)
 *   BOLOUI_LOG_DIR        : override log dir (default <dataDir>/logs)
 *   BOLOUI_STATIC_DIR     : override static dir (default out/renderer)
 *   BOLOUI_BACKEND_BIN    : absolute path to aioncore binary (else PATH lookup)
 *   BOLOUI_BACKEND_BUNDLED_DIR : dir containing bundled-aioncore/<plat-arch>/binary
 *   BOLOUI_OPEN_BROWSER   : "1"/"true" to force open, "0"/"false" to disable
 */
...
const DEFAULT_PORT = (() => {
  ...
  if (process.env.BOLOUI_MULTI_INSTANCE === '1') return 25810;
  return 25809;
})();
 ...
 * work on hosts that never installed BoloUi.app, and its default work dir must
 * NOT collide with Electron's.
 *
 *   --data-dir <path>       CLI override (highest priority)
 *   $BOLOUI_DATA_DIR        env override (same effect)
 *   otherwise               ~/.boloui-web         (production)
 *                           ~/.boloui-web-dev     (dev, default)
 *                           ~/.boloui-web-dev-2   (dev + BOLOUI_MULTI_INSTANCE=1)
 *
 * Why a dedicated `-web` name, not the same `~/.boloui[-dev]` that Electron
 * uses: on macOS, Electron's getDataPath() (packages/desktop/src/process/utils/
 * utils.ts) creates `~/.boloui-dev` as a **symlink** to
 * `~/Library/Application Support/BoloUi-Dev/boloui` so CLI tools (claude,
 ...
 * `.boloui-web` keeps standalone webui's data dir off of the path Electron's
 * symlink needs.
 *
 * If the user wants the two to share data they opt-in explicitly via
 *   --data-dir ~/.boloui-dev                     (or equivalent on other OSes)
...
function resolveBackendDataDir(): string {
  const override = getFlag('--data-dir') ?? process.env.BOLOUI_DATA_DIR;
  ...
  const suffix =
    process.env.NODE_ENV === 'production' ? '' : process.env.BOLOUI_MULTI_INSTANCE === '1' ? '-dev-2' : '-dev';
  const dir = path.join(os.homedir(), `.boloui-web${suffix}`);
...
function resolveAllowRemote(): boolean {
  ...
  const host = process.env.BOLOUI_HOST?.trim();
  if (host && ['0.0.0.0', '::', '::0'].includes(host)) return true;
  return parseBoolean(process.env.BOLOUI_ALLOW_REMOTE ?? process.env.BOLOUI_REMOTE);
}

function resolveStaticDir(): string {
  if (process.env.BOLOUI_STATIC_DIR) return process.env.BOLOUI_STATIC_DIR;
  const candidate = path.join(repoRoot, 'out', 'renderer');
  if (fs.existsSync(path.join(candidate, 'index.html'))) return candidate;
  throw new Error(`Renderer assets not found at ${candidate}. Run "bun run package" first, or set BOLOUI_STATIC_DIR.`);
}
 ...
 *   $BOLOUI_NO_BUILD=1        : env-level opt-out
 *   $BOLOUI_STATIC_DIR is set : caller is pointing us at a prebuilt artifact dir
 */
function runPackageIfNeeded(): void {
  if (has('--no-build')) return;
  if (parseBoolean(process.env.BOLOUI_NO_BUILD)) return;
  if (process.env.BOLOUI_STATIC_DIR) return;
...
function resolveBackendBinary(): string {
  if (process.env.BOLOUI_BACKEND_BIN) return process.env.BOLOUI_BACKEND_BIN;

  const bundledBase = process.env.BOLOUI_BACKEND_BUNDLED_DIR ?? path.join(repoRoot, 'resources', 'bundled-aioncore');
  ...
  throw new Error(
    `Cannot find "${BACKEND_BINARY}". Set BOLOUI_BACKEND_BIN, put it on PATH, or place it at ${bundled}.`
  );
}
...
async function main(): Promise<void> {
  ...
  // This keeps `bun run webui` fully self-contained on hosts without BoloUi.app.
  const workDir = resolveBackendDataDir();
  const staticDir = resolveStaticDir();
  const backendBin = resolveBackendBinary();
  const logDir = process.env.BOLOUI_LOG_DIR ?? path.join(workDir, 'logs');
