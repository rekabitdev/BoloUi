 ...
 * (`/Users/zhoukai/Downloads/boloui-config.txt` + `Archive/*.md`).
 *
 ...
 *
 *   1. Decode the legacy `boloui-config.txt` exactly as ConfigStorage would.
...
const FIXTURE_CONFIG = '/Users/zhoukai/Downloads/boloui-config.txt';
...
function resolveBackendBinary(): string {
  const candidates = [
    process.env.BOLOUI_BACKEND_BINARY,
  ...
  throw new Error('aioncore binary not found (set BOLOUI_BACKEND_BINARY or build it)');
}
...
/**
 * Decode an `boloui-config.txt` exactly the way `JsonFileBuilder` does:
...
describeIfFixtures('migrateAssistantsToBackend (real fixture)', () => {
  ...
  beforeEach(async () => {
    dataDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'boloui-migrate-fixture-'));
