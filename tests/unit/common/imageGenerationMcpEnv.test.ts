...
describe('resolveImageGenerationMcpEnv', () => {
  ...
  it('matches legacy env by platform, base URL, and model when provider id is absent', () => {
    const result = resolveImageGenerationMcpEnv(undefined, [geminiProvider], {
      BOLOUI_IMG_PLATFORM: 'gemini',
      BOLOUI_IMG_BASE_URL: 'https://generativelanguage.googleapis.com/',
      BOLOUI_IMG_MODEL: 'gemini-3-pro-image-preview',
      BOLOUI_IMG_API_KEY: 'stale-key',
    });

    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.source).toBe('field-match');
    expect(result.env.BOLOUI_IMG_PROVIDER_ID).toBe('03c8482c');
    expect(result.env.BOLOUI_IMG_API_KEY).toBe('provider-key');
  });
