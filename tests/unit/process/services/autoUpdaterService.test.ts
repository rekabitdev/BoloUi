...
describe('AutoUpdaterService', () => {
  ...
  beforeEach(() => {
    ...
    appMock.getPath.mockImplementation(() => '/tmp/boloui-test');
    delete (autoUpdaterMock as { updateInfoAndProvider?: unknown }).updateInfoAndProvider;
    appMock.isPackaged = false;
    delete process.env.BOLOUI_FORCE_DEV_AUTO_UPDATE;
    delete process.env.BOLOUI_DEBUG_AUTO_UPDATE_CURRENT_VERSION;
  ...
  it('does not use the stable CDN updater when prerelease manual mode is enabled', async () => {
    autoUpdaterMock.checkForUpdates.mockResolvedValue({
      ...
      updateInfo: {
        version: '2.1.14',
        files: [{ url: 'BoloUi-2.1.14-mac-arm64.dmg', sha512: 'sha512-value' }],
        path: 'BoloUi-2.1.14-mac-arm64.dmg',
        sha512: 'sha512-value',
  ...
  it('ignores a feed version that is not newer than the installed build', async () => {
    ...
    autoUpdaterMock.checkForUpdates.mockResolvedValue({
      ...
      updateInfo: {
        version: '2.1.53',
        files: [{ url: 'BoloUi-2.1.53-mac-arm64.dmg', sha512: 'sha512-value' }],
        path: 'BoloUi-2.1.53-mac-arm64.dmg',
        sha512: 'sha512-value',
        releaseDate: '2026-06-08T00:00:00.000Z',
      },
    });
  ...
  it('overrides the updater current version only for forced unpacked dev checks', async () => {
    process.env.BOLOUI_FORCE_DEV_AUTO_UPDATE = '1';
    process.env.BOLOUI_DEBUG_AUTO_UPDATE_CURRENT_VERSION = '2.1.12';
  ...
  it('restores a completed cached auto-update when the downloaded package validates', async () => {
    const updateInfo = {
      version: '2.1.14',
      files: [{ url: 'BoloUi-2.1.14-mac.zip', sha512: 'sha512-value' }],
      path: 'BoloUi-2.1.14-mac.zip',
      sha512: 'sha512-value',
      releaseDate: '2026-06-08T00:00:00.000Z',
    };
    const fileInfo = {
      url: new URL('https://static.boloui.com/releases/2.1.14/BoloUi-2.1.14-mac.zip'),
      info: { url: 'BoloUi-2.1.14-mac.zip', sha512: 'sha512-value' },
    };
    const cachedUpdatePath = path.join('/cache/pending', 'BoloUi-2.1.14-mac.zip');
  ...
  it('moves the process cwd to temp before the Windows updater handoff', async () => {
    setPlatform('win32');
    const tempRoot = path.join(process.env.TEMP || process.cwd(), `boloui-updater-cwd-test-${process.pid}`);
    const expectedCwd = path.join(tempRoot, 'boloui-updater-cwd');
    const chdir = vi.spyOn(process, 'chdir').mockImplementation(() => undefined);
    appMock.getPath.mockImplementation((name: string) => (name === 'temp' ? tempRoot : '/tmp/boloui-test'));
