...
const makeAppDataDir = () => {
  const dir = mkdtempSync(path.join(tmpdir(), 'boloui-installer-last-failure-'));
  tempDirs.push(dir);
  return dir;
};
...
describe('installerLastFailure service', () => {
  it('consumes a valid installer-last-failure marker once from the AppData BoloUi directory', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        retryCount: 3,
        instDir: 'D:\\BoloUi',
        logPath: 'C:\\Users\\me\\AppData\\Local\\Temp\\boloui-installer-2.1.27-20260702-151830-ab12cd34ef56.log',
        at: '2026-07-01T00:00:00.000Z',
        blockers: [{ pid: 1234, name: 'BoloUi.exe' }],
      })
    );
    ...
    expect(consumed?.blockers).toEqual([{ pid: 1234, name: 'BoloUi.exe' }]);
