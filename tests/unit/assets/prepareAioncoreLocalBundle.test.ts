...
describe('prepare-aioncore local bundle input', () => {
  it('hard fails local bundle input that lacks managed-resources manifest', () => {
    const tmp = mkdtempSync(join(tmpdir(), 'boloui-local-bundle-'));
    ...
    const previous = process.env.BOLOUI_BACKEND_LOCAL_BUNDLE_DIR;
    process.env.BOLOUI_BACKEND_LOCAL_BUNDLE_DIR = localBundle;
    try {
      expect(() =>
        prepareAioncore({
          projectRoot,
      ...
      if (previous === undefined) delete process.env.BOLOUI_BACKEND_LOCAL_BUNDLE_DIR;
      else process.env.BOLOUI_BACKEND_LOCAL_BUNDLE_DIR = previous;
