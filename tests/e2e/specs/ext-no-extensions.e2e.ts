...
const emptyExtensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'boloui-e2e-no-extensions-'));
const stateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'boloui-e2e-no-extensions-state-'));
...
async function launchAppWithoutExtensions(): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      BOLOUI_EXTENSIONS_PATH: emptyExtensionsDir,
      BOLOUI_EXTENSION_STATES_FILE: extensionStatesFile,
