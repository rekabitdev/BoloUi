...
/** Close the feedback modal (BoloModal sets closable=false so Escape is a no-op). */
async function closeFeedbackModal(page: Page) {
  // The feedback modal is an BoloModal (standard variant); its header close
...
/** Close any open BoloModal (e.g. the Agent editor) so the next test starts clean. */
...
async function openCustomAgentEditor(page: Page, command: string) {
  // Defensive: close any BoloModal left over from a prior test so the
...
test('[5] Agent fail_cli alert shows without feedback pill', async ({ page }) => {
  await openCustomAgentEditor(page, 'boloui-e2e-missing-binary-xyz');
