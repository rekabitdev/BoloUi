 ...
 *      (e.g. out/linux-unpacked/boloui, out/mac-arm64/BoloUi.app, out/win-unpacked/BoloUi.exe).
...
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'boloui-e2e-state-'));
...
const e2eUserDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'boloui-e2e-userdata-'));
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/BoloUi.exe  or  out/win-x64-unpacked/BoloUi.exe
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, 'BoloUi.exe');
      if (fs.existsSync(exe)) return { executablePath: exe, cwd: path.join(outDir, dir) };
    }
  } else if (platform === 'darwin') {
    // out/mac-arm64/BoloUi.app/Contents/MacOS/BoloUi  or  out/mac/BoloUi.app/...
    for (const dir of ['mac-arm64', 'mac-x64', 'mac', 'mac-universal']) {
      ...
      if (appBundle) {
        const exe = path.join(macDir, appBundle, 'Contents', 'MacOS', 'BoloUi');
        if (fs.existsSync(exe)) return { executablePath: exe, cwd: macDir };
      }
    }
  } else {
    // Linux: out/linux-unpacked/boloui  (lowercase executable name)
    for (const dir of ['linux-unpacked', 'linux-x64-unpacked', 'linux-arm64-unpacked']) {
      const dirPath = path.join(outDir, dir);
      if (!fs.existsSync(dirPath)) continue;
      // Try common executable names
      for (const name of ['boloui', 'BoloUi']) {
...
async function launchApp(): Promise<ElectronApplication> {
  ...
  const commonEnv = {
    ...process.env,
    BOLOUI_EXTENSIONS_PATH: process.env.BOLOUI_EXTENSIONS_PATH || path.join(projectRoot, 'examples'),
    BOLOUI_EXTENSION_STATES_FILE: process.env.BOLOUI_EXTENSION_STATES_FILE || e2eStateFile,
    BOLOUI_DISABLE_AUTO_UPDATE: '1',
    BOLOUI_DISABLE_DEVTOOLS: '1',
    BOLOUI_E2E_TEST: '1',
    BOLOUI_E2E_USER_DATA_DIR: process.env.BOLOUI_E2E_USER_DATA_DIR || e2eUserDataDir,
    /**
     ...
     */
    BOLOUI_CDP_PORT: process.env.BOLOUI_CDP_PORT || '9230',
  };
