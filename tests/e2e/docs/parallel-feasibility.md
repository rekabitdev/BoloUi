...
**Critical bottleneck**: `workers: 1` enforced + shared `boloui.db` → parallel execution impossible without refactoring.
...
1. Extend `BOLOUI_E2E_TEST` to include worker ID:
   ```typescript
   BOLOUI_E2E_TEST_WORKER_ID: process.env.PLAYWRIGHT_WORKER_INDEX || '0';
   ```
2. Modify `getDevAppName()` to return worker-specific name:
   ```typescript
   const workerId = process.env.BOLOUI_E2E_TEST_WORKER_ID || '0';
   return `BoloUi-E2E-Worker-${workerId}`;
   ```
3. Each worker gets isolated:
   - `~/Library/Application Support/BoloUi-E2E-Worker-0/config/boloui.db`
   - `~/Library/Application Support/BoloUi-E2E-Worker-1/config/boloui.db`
...
**Complexity**: High (requires main process to read `BOLOUI_DATABASE_PATH`, conflicts with userData convention)
