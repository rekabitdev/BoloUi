import { startWebHost, startStaticServer } from '@boloui/web-host';
import type { WebHostHandle, StaticServerHandle } from '@boloui/web-host';
...
// tarball layout:
//   boloui-web/
//   ├── boloui-web              ← bun-compiled standalone binary (process.execPath)
...
const isPackaged = (() => {
  ...
  return exeName === 'boloui-web' || exeName === 'boloui-web.exe';
})();

const BACKEND_BINARY = process.platform === 'win32' ? 'aioncore.exe' : 'aioncore';
const DEFAULT_PORT = 25808;
const RESET_COMMAND = isPackaged ? 'boloui-web resetpass' : 'bun run resetpass';
...
function resolveBackendBinary(flags: Map<string, string | true>): string {
  ...
  const envOverride = process.env.BOLOUI_BACKEND_BIN;
...
function resolveDataDir(flags: Map<string, string | true>): string {
  ...
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.BOLOUI_DATA_DIR;
  if (envOverride) return path.resolve(envOverride);
  return path.join(os.homedir(), '.boloui-web');
}

function resolveLogDir(flags: Map<string, string | true>, dataDir: string): string {
  const override = flags.get('log-dir');
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.BOLOUI_LOG_DIR;
  if (envOverride) return path.resolve(envOverride);
...
function resolvePort(flags: Map<string, string | true>): number {
  ...
  const env = process.env.BOLOUI_PORT ?? process.env.PORT;
...
function resolveAllowRemote(flags: Map<string, string | true>): boolean {
  ...
  const env = process.env.BOLOUI_ALLOW_REMOTE ?? process.env.BOLOUI_REMOTE;
  if (!env) return false;
...
async function runStart(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(staticDir)) {
    console.error(`[boloui-web] static dir not found: ${staticDir}`);
    console.error(`  hint: pass --static-dir <path> pointing to the SPA build output`);
    process.exit(1);
  }

  console.log(`[boloui-web] version    : ${version}`);
  console.log(`[boloui-web] data dir   : ${dataDir}`);
  console.log(`[boloui-web] log dir    : ${logDir}`);
  console.log(`[boloui-web] static dir : ${staticDir}`);
  console.log(`[boloui-web] backend bin: ${backendBin}`);
  console.log(`[boloui-web] launching  : port=${port} allowRemote=${allowRemote}`);
  ...
  if (!backendAvailable) {
    ...
    console.warn('   To enable backend: download aioncore and set BOLOUI_BACKEND_BIN.');
    console.warn('');
    ...
    console.log('');
    console.log('BoloUi WebUI (frontend only) is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
    if (autoOpenBrowser) {
      const openResult = openBrowserUrl(handle.localUrl);
      if (openResult.ok) {
        console.log(`[boloui-web] opened ${handle.localUrl} in your browser.`);
      } else {
        console.warn(`[boloui-web] could not open the browser automatically: ${openResult.reason}`);
      }
    }
    console.log('');
    ...
    console.log('');
    console.log('BoloUi WebUI is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
  ...
  const shutdown = async (signal: string): Promise<void> => {
    ...
    shuttingDown = true;
    console.log(`\n[boloui-web] received ${signal}, stopping...`);
    try {
      if (currentHandle) await currentHandle.stop();
    } catch (err) {
      console.error('[boloui-web] stop failed:', err);
    }
    process.exit(0);
  };
...
/**
 * `boloui-web resetpass` — spin up the backend just long enough to POST
...
async function runResetPassword(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(backendBin)) {
    console.error(`[boloui-web] backend binary not found: ${backendBin}`);
    console.error('  hint: pass --backend-bin <path> or set BOLOUI_BACKEND_BIN');
    process.exit(1);
  }
  const dataDir = resolveDataDir(flags);
  fs.mkdirSync(dataDir, { recursive: true });
  ...
  console.log(`[boloui-web] resetting admin password in ${dataDir}`);
  ...
  try {
    ...
    if (!ready) {
      console.error('[boloui-web] backend did not become ready within 15s');
      process.exit(1);
    }
    ...
    if (!res.ok) {
      console.error(`[boloui-web] /api/webui/reset-password returned ${res.status}`);
      process.exit(1);
    }
    ...
    if (!newPassword) {
      console.error('[boloui-web] reset-password response missing new_password');
      process.exit(1);
    }
    console.log(`[boloui-web] username: ${username}`);
    console.log(`[boloui-web] new password: ${newPassword}`);
    console.log('[boloui-web] existing sessions have been invalidated.');
  } finally {
    try {
      await handle.stop();
    } catch {
...
async function main(): Promise<void> {
  ...
  if (command === '--help' || command === 'help' || command === '-h') {
    console.log(`Usage: boloui-web <command> [options]
...
Options for start:
  ...
  --data-dir <path>       Override data dir (default: ~/.boloui-web)
...
Options for resetpass:
  --data-dir <path>       Which data dir to reset (default: ~/.boloui-web)
  --backend-bin <path>    Override backend binary path

Environment variables:
  BOLOUI_PORT, BOLOUI_ALLOW_REMOTE, BOLOUI_DATA_DIR, BOLOUI_LOG_DIR,
  BOLOUI_BACKEND_BIN, BOLOUI_OPEN_BROWSER
`);
    return;
  }
  ...
  if (command !== 'start') {
    ...
    console.error('Usage: boloui-web [start|resetpass|version|help]');
    process.exit(1);
  }
...
main().catch((err: Error) => {
  console.error('[boloui-web] fatal:', err.message);
