...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'boloui');
};
 ...
 * Release builds use ~/.boloui; dev builds use ~/.boloui-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.boloui，Dev 模式使用 ~/.boloui-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, 'boloui');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.boloui'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.boloui-config; dev builds use ~/.boloui-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.boloui-config，Dev 模式使用 ~/.boloui-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.boloui-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[BoloUi] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[BoloUi] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[BoloUi] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[BoloUi] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
