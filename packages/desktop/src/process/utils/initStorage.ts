...
const STORAGE_PATH = {
  config: 'boloui-config.txt',
  chatMessage: 'boloui-chat-message.txt',
  chat: 'boloui-chat.txt',
  env: '.boloui-env',
...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[BoloUi] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[BoloUi] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[BoloUi] 数据迁移失败:', error);
  }
...
const dirConfig = envFile.getSync('boloui.dir');
...
const buildMessageListStorage = (conversation_id: string, dir: string) => {
  const fullName = path.join(dir, 'boloui-chat-history', conversation_id + '.txt');
  if (!existsSync(fullName)) {
    mkdirSync(path.join(dir, 'boloui-chat-history'));
  }
  return JsonFileBuilder<TMessage[]>(path.join(dir, 'boloui-chat-history', conversation_id + '.txt'));
};
  ...
  return {
    ...
    backup(conversation_id: string) {
      ...
      return storage.backup(
        path.join(dir, 'boloui-chat-history', 'backup', conversation_id + '_' + Date.now() + '.txt')
      );
    },
  };
};
...
const cleanupLegacyBuiltinSkillsDir = () => {
  ...
  fs.rm(legacyDir, { recursive: true, force: true })
    .then(() => console.log('[BoloUi] Cleaned up legacy builtin-skills cache'))
    .catch(() => {
 ...
 * `crates/boloui-app/assets/builtin-assistants/` and
 * `crates/boloui-app/assets/builtin-skills/`) — neither is synced from
...
const initStorage = async () => {
  ...
  const mark = (label: string) => console.log(`[BoloUi:init] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  //    now live in the backend SQLite catalog (see boloui-assistant crate) and
  ...
  try {
    ...
    console.error('[BoloUi] Failed to ensure assistant dirs:', error);
  }
