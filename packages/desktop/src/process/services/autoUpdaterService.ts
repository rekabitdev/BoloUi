...
const FORCE_DEV_AUTO_UPDATE_ENV = 'BOLOUI_FORCE_DEV_AUTO_UPDATE';
...
class AutoUpdaterService extends EventEmitter {
  ...
  private ensureDevUpdateConfig(): void {
    try {
      ...
      const devConfig = [
        'provider: generic',
        `url: ${cdnFeedOptions.url}`,
        'updaterCacheDirName: com.boloui.app',
        '',
      ].join('\n');
  ...
  private moveCwdOutOfInstallDirForWindowsHandoff(): void {
    ...
    try {
      const safeCwd = path.join(app.getPath('temp'), 'boloui-updater-cwd');
   ...
   * while the downloaded archive contains the packaged app (com.boloui.app). Squirrel.Mac
