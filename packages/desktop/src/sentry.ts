...
function isBackendStartupFailureEvent(event: { tags?: Record<string, unknown> }): boolean {
  return event.tags?.['boloui.failure'] === 'backend_startup';
}

function isUserFeedbackEvent(event: { tags?: Record<string, unknown> }): boolean {
  return event.tags?.type === 'user-feedback' || event.tags?.['boloui.installation_integrity.user_report'] === 'true';
}
...
function getInstallPathKind(resourcesPath: unknown): string | undefined {
  ...
  if (normalized.includes('\\appdata\\local\\programs\\boloui\\resources')) {
    return 'user_local_programs';
  }
  if (
    normalized.includes('\\program files\\boloui\\resources') ||
    normalized.includes('\\program files (x86)\\boloui\\resources')
  ) {
...
export async function captureBackendStartupFailure(error: unknown): Promise<void> {
  ...
  Sentry.withScope((scope) => {
    scope.setTag('boloui.failure', 'backend_startup');
    scope.setTag('boloui.backend_startup.reason', failureInfo.reason);
    if (failureInfo.runtime) {
      scope.setTag('boloui.backend_startup.runtime', failureInfo.runtime);
    }
    if (failureInfo.packageArch) {
      scope.setTag('boloui.backend_startup.package_arch', failureInfo.packageArch);
    }
    if (failureInfo.deviceArch) {
      scope.setTag('boloui.backend_startup.device_arch', failureInfo.deviceArch);
    }
    if (failureInfo.expectedDownloadArch) {
      scope.setTag('boloui.backend_startup.expected_download_arch', failureInfo.expectedDownloadArch);
    }
    if (typeof failureInfo.isRosettaTranslated === 'boolean') {
      scope.setTag('boloui.backend_startup.rosetta_translated', getBooleanTagValue(failureInfo.isRosettaTranslated));
    }
    if (typeof details?.stage === 'string') {
      scope.setTag('boloui.backend_startup.stage', details.stage);
    }
    if (failureInfo.backendBoundaryCode) {
      scope.setTag('boloui.backend_startup.boundary_code', failureInfo.backendBoundaryCode);
    }
    if (failureInfo.backendBoundaryStage) {
      scope.setTag('boloui.backend_startup.boundary_stage', failureInfo.backendBoundaryStage);
    }
    if (failureInfo.localDataIssueKind) {
      scope.setTag('boloui.backend_startup.local_data_issue_kind', failureInfo.localDataIssueKind);
    }
    if (failureInfo.incompleteInstallationKind) {
      scope.setTag('boloui.backend_startup.incomplete_installation_kind', failureInfo.incompleteInstallationKind);
    }
    for (const [tag, value] of [
      ['boloui.backend_startup.missing_bundled_dir', getBooleanTagValue(failureInfo.missingBundledAioncoreDir)],
      ['boloui.backend_startup.missing_runtime_dir', getBooleanTagValue(failureInfo.missingRuntimeDir)],
      ['boloui.backend_startup.missing_binary', getBooleanTagValue(failureInfo.missingBackendBinary)],
      ['boloui.backend_startup.missing_hub_dir', getBooleanTagValue(failureInfo.missingHubDir)],
      ['boloui.backend_startup.missing_pet_states_dir', getBooleanTagValue(failureInfo.missingPetStatesDir)],
      ['boloui.backend_startup.missing_pwa_dir', getBooleanTagValue(failureInfo.missingPwaDir)],
      ['boloui.backend_startup.install_path_kind', getInstallPathKind(details?.resourcesPath)],
      ['boloui.backend_startup.last_update_status', getString(autoUpdateDiagnostics?.lastEvent?.status)],
      [
        'boloui.backend_startup.health_polling_delayed',
        getBooleanTagValue(
          typeof details?.healthCheckPollingDelayed === 'boolean' ? details.healthCheckPollingDelayed : undefined
        ),
      ],
      ['boloui.backend_startup.health_attempts_bucket', getHealthAttemptBucket(details?.healthCheckAttempts)],
      [
        'boloui.backend_startup.health_attempt_deficit_bucket',
        getHealthAttemptBucket(details?.healthCheckAttemptDeficit),
      ],
      ['boloui.backend_startup.health_timeout_overrun_bucket', getDurationBucket(details?.healthCheckTimeoutOverrunMs)],
      ['boloui.backend_startup.health_max_attempt_gap_bucket', getDurationBucket(details?.healthCheckMaxAttemptGapMs)],
      [
        'boloui.backend_startup.seconds_since_quit_and_install',
