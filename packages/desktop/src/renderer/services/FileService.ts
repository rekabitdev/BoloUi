...
import { BOLOUI_TIMESTAMP_REGEX } from '@/common/config/constants';
import { formatByteSize } from '@/renderer/services/i18n/format';

// 清理BoloUi时间戳后缀，返回原始文件名
export function cleanBoloUiTimestamp(file_name: string): string {
  return file_name.replace(BOLOUI_TIMESTAMP_REGEX, '$1');
}

// 从文件路径获取清理后的文件名（用于UI显示）
export function getCleanFileName(file_path: string): string {
  const file_name = file_path.split(/[\\/]/).pop() || '';
  return cleanBoloUiTimestamp(file_name);
}
...
class FileServiceClass {
   ...
   * directory (`temp_dir/boloui/...`). Passing the raw device path (the old
