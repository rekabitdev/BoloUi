...
/** Backend manifest id of the built-in BoloUi Butler assistant. */
const BUTLER_ASSISTANT_ID = 'boloui-assistant';
 ...
 * select the BoloUi Butler, and pre-fill the chat input with a ready-made
      ...
      try {
        ...
        if (butler) {
          ...
          if (butler.enabled === false) {
            ...
            Message.success(
              t('settings.talkToButler.enabledToast', { defaultValue: 'Enabled the BoloUi Butler for you' })
            );
          }
        }
      } catch (error) {
