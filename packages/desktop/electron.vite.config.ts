...
// Read the real BoloUi version from the repo-root package.json.
...
const mainAliases = {
  ...
  '@boloui/web-host': resolve('packages/web-host/src/index.ts'),
  ...
  return {
    main: {
      plugins: [
        ...
        // '@boloui/web-host' excluded so its TS sources (which use ESM ".js" import specifiers)
        // are bundled by esbuild rather than left as `require('@boloui/web-host')`, which Node
        // cannot resolve because the package ships no compiled .js files (workspace-only).
        externalizeDepsPlugin({ exclude: ['fix-path', '@boloui/web-host'] }),
        ...(isDevelopment
          ? [
              {
      ...
      define: {
        ...
        // Discontinued-build fork flag (see discontinuedBuild.ts). Only BoloUi's
    ...
    renderer: {
      ...
      appType: 'mpa',
      server: {
        // Default to 5173; when occupied (e.g. another BoloUi clone is running),
      ...
      define: {
        'process.env.NODE_ENV': JSON.stringify(mode),
        'process.env.env': JSON.stringify(process.env.env),
        'process.env.BOLOUI_MULTI_INSTANCE': JSON.stringify(process.env.BOLOUI_MULTI_INSTANCE ?? ''),
        'process.env.SENTRY_DSN': JSON.stringify(process.env.SENTRY_DSN ?? ''),
        // Inject the real BoloUi version (root package.json) so renderer code
