...
BoloUi development uses two repositories:
...
- **BoloUi** (`https://github.com/iOfficeAI/BoloUi.git`) starts the Electron desktop app and launches the backend binary automatically.
...
The desktop development server resolves the backend from the `PATH` inherited by `bun run start`. Install AionCore first, verify the binary is discoverable in the same terminal, then start BoloUi.
...
git clone https://github.com/iOfficeAI/BoloUi.git
```
...
cd AionCore
cargo clean
cargo install --path crates/boloui-app --locked

# Make Cargo-installed binaries visible to this shell if needed.
export PATH="$HOME/.cargo/bin:$PATH"

# Verify that BoloUi will be able to find the backend.
which aioncore
aioncore --help
```
...
Run these commands from the `BoloUi` repository in a terminal where `aioncore` is discoverable.
...
During startup, BoloUi launches `aioncore` automatically and passes the backend port to the renderer. You do not need to start AionCore in a separate terminal.

## Updating the Local Backend

When you pull or change AionCore, reinstall the backend binary and restart BoloUi:

```bash
cd ../AionCore
cargo install --path crates/boloui-app --locked --force
...
BoloUi cannot find the backend from the `PATH` inherited by `bun run start`.

Check from the same terminal where you start BoloUi:
...
If the command fails, add Cargo's binary directory to `PATH` and start BoloUi from a new terminal.

### `aioncore` Works in a Terminal but BoloUi Still Cannot Find It
...
Quit BoloUi, reinstall AionCore with `cargo install --path crates/boloui-app --locked --force`, then start BoloUi again. The Electron app owns the backend subprocess during development, so a running BoloUi instance will not pick up a newly installed binary until it restarts.
...
When you have two clones of the repository (e.g. `BoloUi` and `BoloUi-refactor`) and need to run both simultaneously, the second instance can be started with:

```bash
bun run start:multi
```

This sets `BOLOUI_MULTI_INSTANCE=1`, which:

- Skips the Electron single-instance lock
- Uses a separate userData directory (`BoloUi-Dev-2`) to avoid database and config conflicts
- Isolates data/config symlink paths (`~/.boloui-dev-2`, `~/.boloui-config-dev-2`)
...
BoloUi uses **electron-vite** for fast bundling:
